...
const Project = (props) => {
  ...
  const filteredData = userData.filter((el)=>{
    if(inputText == ""){
      ...
      return el.project_title.toLowerCase().includes(inputText);
    }
  })
                    ...
                    <List>
                      <ListItem >
                          ...
                          >
                            ...
                            title={`Delete ${value['project_title']}`}
                            
                            >
                            <IconButton onClick={()=>{setSelectedID(value['id']);setSelectedTitle(value['project_title']);setShowModal(true); }} 
                            spacing={200} 
                            aria-label="delete"
                            >
                              <DeleteOutlineIcon 
